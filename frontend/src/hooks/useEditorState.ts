...
import { listPanels, createPanel, deletePanel as apiDeletePanel } from "@/lib/panelApi";
...
export function useEditorState(initialProjectId?: string | null) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (isValidUUID(cutId)) {
        ...
        if (!saved) {
          saved = await saveCutDataToBackend(cutId, payload);
        }
        if (!saved) {
  ...
  const deleteCut = useCallback(async (cutId: string) => {
    ...
    // Supabase cut_data 정리
    try {
      await supabase.from("cut_data").delete().eq("cut_id", cutId);
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error("[useEditorState] cut_data 삭제 실패:", err);
    }
