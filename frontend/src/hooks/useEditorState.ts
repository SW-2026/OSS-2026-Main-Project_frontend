  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (isValidUUID(activeCutId)) {
        // UUID 형식이면 Supabase에 저장
        await supabase.from("cut_data").upsert(payload, { onConflict: "cut_id" });
      } else {
        // UUID가 아니면 localStorage에 저장
        localStorage.setItem(getCutDataKey(activeCutId), JSON.stringify(payload));
      }
